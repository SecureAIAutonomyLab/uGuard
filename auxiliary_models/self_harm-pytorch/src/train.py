...
def train(train_loader, model, criterion, optimizer, writer, batch_size, epoch, step, n):
    ...
    for i, (images, labels, name) in enumerate(train_loader):
        ...
        labels = Variable(labels).cuda()
        label_pred = model(images)
        # one_hot_labels = nn.functional.one_hot(labels, num_classes=2).cuda().float()#torch.zeros((len(labels), label_pred.shape[-1]), device='cuda')
        
        # BCE Loss With Logits
        
        loss = criterion(label_pred, labels)
        # print("Labels pred: {}; Label True: {}; Loss: {}".format(label_pred, labels, loss.item()))
