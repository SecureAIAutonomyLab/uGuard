...
def evaluate(eval_loader, model, args):
    ...
    for i, (images, labels) in tqdm.tqdm(enumerate(eval_loader)):
        ...
        if labels==0 and class_0>max_samples:
            continue
        if labels==1 and class_1>max_samples:
            continue
        images = get_random_attack_simple(model, images, labels)
        save_name = "image_"+str(i).zfill(5)+".png"
        if labels == 0:
            torchvision.utils.save_image(images, "../../datasets/nsfw_attacked_simple/train/neutral/"+ save_name)
            class_0 +=1
        else:
            torchvision.utils.save_image(images, '../../datasets/nsfw_attacked_simple/train/porn/'+ save_name)
            class_1 +=1
