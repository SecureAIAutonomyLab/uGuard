...
class yahoo_call:
    def __init__(self, input_path: str):
        ...
        def load_preprocess(path):
            ...
            return input_img
        #-------------------------------------------------------------------------------------------------------------
        onsfw_model = make_open_nsfw_model()
        ...
        for file_path in input_files:
            test_img = load_preprocess(os.path.join(input_path, file_path))
            likelihood = onsfw_model.predict(test_img)
            results.append({
                'file': file_path,
                'likelihood': str(likelihood[0][1])
            })
        print(results)
        # save as json file
        with open(os.path.join(results_path, "yahoo_results.json"), 'w') as f_out:
